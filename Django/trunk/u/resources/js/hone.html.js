...
function rr_renren(data) {
	result=data.result
	$.each(result,function(i,v) {
		threads.push({type:"renren",time:date_renren_to_standard(v.update_time),content:v})
	})
	current+=1
	if (current==2) {
		finalize()
...
function rr_weibo(data) {
	result=data.result
	$.each(result.statuses,function(i,v) {
		threads.push({type:"wb",time:date_wb_to_standard(v.created_at),content:v})
	})
	current+=1
	if (current==2) {
		finalize()
...
function finalize() {
	bubbleSort(threads,sortStatus)
	$.each(threads,function(i,v) {
		console.log(v.time)
		switch (v.type) {
			case "wb":
				wb_handler(v.content);
				break;
			case "renren":
				renren_handler(v.content);
				break;
		}
	})
}
...
Dajaxice.dajax_lib.call_api(rr_renren,{'request_method':"renren.feed.get",'token':'{{result.renren_access_token}}'})
Dajaxice.dajax_lib.call_api(rr_weibo,{'request_method':"wb.statuses.home_timeline",'token':'{{result.wb_access_token}}'})